...
#----NDVI pixels plot for all blocks and datasets----
...
#full timeseries plot (created and assembled with patchwork)
{
  #NN
  {
    nn_plot = ggplot(subset_df |> filter(dataset=='NN'))+
      #show 5th to 95th percentiles for pixel strata
      # geom_ribbon(aes(x = acquired, ymin = X5., ymax = X95., fill = block_pixel_stratum, colour = block_pixel_stratum), alpha = 0.1)+
      #show interquartile range of pixel strata
      geom_ribbon(aes(x = acquired, ymin = X25., ymax = X75., fill = block_pixel_stratum, color=block_pixel_stratum), alpha = 0.3)+
  ...
  #SM
  {
    sm_plot = ggplot(subset_df |> filter(dataset=='SM'))+
      #show 5th to 95th percentiles for pixel strata
      # geom_ribbon(aes(x = acquired, ymin = X5., ymax = X95., fill = block_pixel_stratum, colour = block_pixel_stratum), alpha = 0.1)+
      #show interquartile range of pixel strata
      geom_ribbon(aes(x = acquired, ymin = X25., ymax = X75., fill = block_pixel_stratum, color=block_pixel_stratum), alpha = 0.3)+
  ...
  #Z
  {
    z_plot = ggplot(subset_df |> filter(dataset=='Z'))+
      #show 5th to 95th percentiles for pixel strata
      # geom_ribbon(aes(x = acquired, ymin = X5., ymax = X95., fill = block_pixel_stratum, colour = block_pixel_stratum), alpha = 0.1)+
      #show interquartile range of pixel strata
      geom_ribbon(aes(x = acquired, ymin = X25., ymax = X75., fill = block_pixel_stratum, color=block_pixel_stratum), alpha = 0.3)+
  ...
  #Zr
  {
    zr_plot = ggplot(subset_df |> filter(dataset=='Zr'))+
      #show 5th to 95th percentiles for pixel strata
      # geom_ribbon(aes(x = acquired, ymin = X5., ymax = X95., fill = block_pixel_stratum, colour = block_pixel_stratum), alpha = 0.1)+
      #show interquartile range of pixel strata
      geom_ribbon(aes(x = acquired, ymin = X25., ymax = X75., fill = block_pixel_stratum, color=block_pixel_stratum), alpha = 0.3)+
  ...
  combined_plot = (nn_plot | sm_plot | z_plot | zr_plot) + plot_layout(guides = 'collect')
  combined_plot
...
#plots for presentation
{
  ...
  #Zr
  {
    zr_plot = ggplot(subset_df |> filter(dataset=='Zr'))+
      ...
      #show vertical lines for start and end of harvesting
      geom_vline(data = harvest_dates_df[harvest_dates_df$block_id %in% unique(subset_df$block_id),], aes(xintercept = harvest_finish_date, linetype = 'Harvest end date'),color='blue')+
      geom_vline(data = harvest_dates_df[harvest_dates_df$block_id %in% unique(subset_df$block_id),] ,aes(xintercept = harvest_start_date, linetype = 'Harvest start date'),color='blue')+
      scale_linetype_manual(
        name = "Harvest date",
        values = c(
          "Harvest start date" = "dotted",
