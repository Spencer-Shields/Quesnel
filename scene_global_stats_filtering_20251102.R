...
pre_lidar_files = list.files(lid_mets_pre_cropped_dir, full.names = T) #pre-harvest lidar data
names(pre_lidar_files) = basename(file_path_sans_ext(pre_lidar_files))

post_lidar_files = list.files(lid_mets_post_dec_cropped_dir, full.names = T) #decimated post-harvest lidar data
names(post_lidar_files) = basename(file_path_sans_ext(post_lidar_files))

#----extract stats from each raster and save----
tree_height = 5 #definition of tree height used to detect thinning

harvest_threshold = paste0(tree_height,"m_using_pre&post")

data_string = paste0('Stats_percentiles_JMD_HarvestThreshold=',harvest_threshold,'_Res=',target_res,'_20251102')
...
if(!file.exists(data_filename)){
  ...
  #load lidar CHM layers
  lid_masks = pblapply(block_ids, function(b){
    
    cat('\r',b)
    ##load lidar rasters, get canopy height
    pre_r = rast(pre_lidar_files[[b]]) |> subset('z_p95')
    post_r = rast(post_lidar_files[[b]]) |> subset('z_p95')
    
    #make sure rasters line up
    if(any(
      sapply(1:4, function(i){ext(pre_r)[i]<ext(post_r)[1]})
    )
    ){pre_r = extend(pre_r,post_r)}
    
    if(any(
      sapply(1:4, function(i){ext(post_r)[i]<ext(pre_r)[1]})
    )
    ...
    #make thematic thinning/non-thinning layer
    mpre = ifel(pre_r >= tree_height, 1, 0)
    mpost = ifel(post_r < tree_height, 1, 0)
    thin = mpre & mpost
    
    #separate into stack of thinning/nonthinning mask layers
    m_t = ifel(thin == 1, 1, NA) #create thinning mask layer
    m_nt = ifel(thin == 0, 0, NA) #create non-thinning mask layer
    m = c(m_t, m_nt) #stack layers
    names(m) = c('Thinning', 'Non_thinning') #name layers
    
    #extra steps to make balanced classes
    {
...
hq_scenes_permonth_summ = results_meta_df |>
  select(dataset, block_id, id, acquisition_date, acquisition_month) |> distinct() |>
...
month_labels <- hq_scenes_permonth_summ$year_mon[hq_scenes_permonth_summ$acquisition_month %in% c(1, 7)]

ggplot(hq_scenes_permonth_summ #|> filter(str_detect(dataset, 'SM'))
       , aes(x = year_mon, y = count, fill=block_id))+
  geom_col(
    # fill = NA
    )+
  # geom_line()+
