...
otsu_df = read_csv("data/Quesnel_thinning/Otsu_3m_change_thresholds.csv") #use otsu thresholds calculated based on chm resampled to match PS scenes
...
#----timeseries analysis by thinned vs not-thinned pixels----
{
  ...
  data_string = paste0('REDO_Stats_logAUC_JMD_HarvestThreshold=',harvest_threshold,'_EqualClasses')
  ...
  results_df = results_df |>
    ...
    mutate(months_since_harvest = as.numeric(acquisition_date - harvest_start_date)) |>
  ...
  # hq_vl = pblapply(hq_rl, function(x)values(x,dataframe=T)|>select(all_of(feats_to_use)))
  # hq_v = rbindlist(hq_vl) |>
  #   # select(-EVI2,-NRVI,-VARIgreen) |> 
  #   na.omit()
  # nrow(hq_v)
  # 
  ...
  #pixels plot
  {
    z_pix = ggplot(data = results_12N_T3 |> filter(dataset=='Z_REDO'),
      ...
      ggtitle('Z-score')+
      theme_classic()
    zr_pix = ggplot(data = results_12N_T3 |> filter(dataset=='Zrobust_REDO'),
                    aes(x = acquisition_date,y=median))+
      # geom_bin_2d(aes(x = acquisition_date, y = mean, fill = block_pixel_stratum),alpha=0.5)+
      geom_point(aes(color = block_pixel_stratum),alpha=0.5)+
      #geom_ribbon(aes(ymin = mean - sd, ymax = mean + sd, fill = block_pixel_stratum), alpha = 0.2, color = NA) +
      ...
      theme_classic()
    nn_pix = ggplot(data = results_12N_T3 |> filter(dataset=='Non-normalized_REDO'),
                    aes(x = acquisition_date,y=median))+
      # geom_bin_2d(aes(x = acquisition_date, y = mean, fill = block_pixel_stratum),alpha=0.5)+
      geom_point(aes(color = block_pixel_stratum),alpha=0.5)+
      #geom_ribbon(aes(ymin = mean - sd, ymax = mean + sd, fill = block_pixel_stratum), alpha = 0.2, color = NA) +
      ...
      theme_classic()
    sm_pix = ggplot(data = results_12N_T3 |> filter(dataset=='SM_REDO'),
                    aes(x = acquisition_date,y=median))+
      # geom_bin_2d(aes(x = acquisition_date, y = mean, fill = block_pixel_stratum),alpha=0.5)+
      geom_point(aes(color = block_pixel_stratum),alpha=0.5)+
      #geom_ribbon(aes(ymin = mean - sd, ymax = mean + sd, fill = block_pixel_stratum), alpha = 0.2, color = NA) +
      ...
      theme_classic()
    library(patchwork)
    nn_pix+z_pix+zr_pix+sm_pix+plot_layout(guides = 'collect',ncol=4)
    }
  
  #logistic AUC plot
  z_log = ggplot(data = results_12N_T3 |> 
                   filter(dataset == 'Z'), 
  ...
  #set up directories
  rf_dir = paste0(ps_dir,'/REDO_random_forest_models_nopredict','_',harvest_threshold)
  dir.check(rf_dir)
  ...
  pblapply(all_files_12nt3, function(f){
  ...
  #----do random forest for each date for each block----
  
  results_all_filt = results_meta_df |>
    # filter(notNA >= 0.8*max_valid_pixels) |>
    filter(
      # block_id == '12N_T3'
      #      ,id %in% id_df_12N_T3$id,
  ...
  #set up directories
  rf_dir = paste0(ps_dir,'/REDO_random_forest_models_nopredict'
                  ,'_',harvest_threshold
                  )
  dir.check(rf_dir)
