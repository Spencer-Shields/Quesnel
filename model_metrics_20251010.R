...
pbmapply(function(prehid, posthid){
  ...
  print('Set lidar metrics to predict and spectral/texture features to use')
  #lidar
  mets_to_test = c( 
    "z_above2","z_p95","z_sd"
    # ,
  ...
  #ps spectral
  spec.feats3 = c('SAVI', 'VARIgreen', 'SR', 'NDGR', 'GCC', 'YNDVI')
  ...
  metric_predictor_fn3 = function(met, ps_scene,
  ...
  feat_set = 'FeatureSet3_1stepVSURF'
  ...
  pblapply(mets_to_test, function(m){
    pblapply(unique(results_meta_df$dataset), function(d){
      ...
      if(!file.exists(f)){
        ...
        mod = metric_predictor_fn3(met = m, ps_scene = ps_change, glcm_rast = glcm_c)
        write_rds(mod, f)
