...
chm_post_ids = file_path_sans_ext(list.files(chm_post_dir)) %>% str_extract(".*(?=_chm_)")
chm_post_ids = chm_post_ids[!is.na(chm_post_ids)]
chm_pre_ids = file_path_sans_ext(list.files(chm_pre_dir)) %>% str_extract(".*(?=_chm_)")
chm_pre_ids = chm_pre_ids[!is.na(chm_pre_ids)]
...
change_rasts_l = pblapply(X = 1:length(post_rasts_common_l), function(i){
  ...
  #calculate change
  change_rast = pre_rast - post_rast
